...
Begin VB.Form frmAttackResponse 
   ...
   Begin MSComDlg.CommonDialog cdgOpen 
      ...
      Filter          =   "ATK responses|*.txt|All Files|*.*"
   End
...
Public Sub PrepareTabs()
    ...
    If LenB(LastResponse) <> 0 Then
        ...
        If ActivateSuggestions = True Then
            If (Dir$(SuggestionsDirectory, 16) <> "") = True Then
                'Define the suggestions directory
                filSuggestions.Path = SuggestionsDirectory
...
Private Sub SelectPluginTrigger()
    ...
    If LenB(LastResponse) > LenB(plugin_trigger) Then
        Dim Response() As String

        'Split the response apart
        Response = Split(LastResponse, plugin_trigger)

        'Select the trigger text
        txtLastResponse.SelStart = Len(Response(0))
        txtLastResponse.SelLength = Len(plugin_trigger)
...
Private Sub DoSilentChecksScan()
    ...
    Call ParseATKPlugin(ReadPluginFromFile(OriginalPluginFilename))
End Sub
...
Private Sub lsvSilentPlugins_ColumnClick(ByVal ColumnHeader As MSComctlLib.ColumnHeader)
    If lsvSilentPlugins.SortKey = ColumnHeader.Index - 1 Then
        If lsvSilentPlugins.SortOrder = lvwAscending Then
            lsvSilentPlugins.SortKey = ColumnHeader.Index - 1
            lsvSilentPlugins.SortOrder = lvwDescending
        Else
            lsvSilentPlugins.SortKey = ColumnHeader.Index - 1
            lsvSilentPlugins.SortOrder = lvwAscending
        End If
    Else
        lsvSilentPlugins.SortKey = ColumnHeader.Index - 1
        lsvSilentPlugins.SortOrder = lvwAscending
...
Private Sub lsvSuggestions_Click()
    ...
    If lsvSuggestions.ListItems.Count > 0 Then
        ...
        lblSuggestion.Caption = lsvSuggestions.SelectedItem.SubItems(1) & _
            vbCrLf & vbCrLf & lsvSuggestions.SelectedItem.SubItems(2)
    End If
End Sub
...
Private Sub mnuFileOpenItem_Click()
    ...
    cdgOpen.InitDir = ResponseDirectory
    ...
    If LenB(ResponseFileName) <> 0 Then
        ...
        If (Dir$(ResponseFileName, 16) <> "") = True Then
...
Private Function OpenResponseFile(ResponseFileName As String)
    ...
    Open ResponseFileName For Input As 1
        Do While Not EOF(1)
            Line Input #1, Temp
                LastResponse = LastResponse & Temp & vbCrLf
        Loop
    Close
...
Private Sub mnuSuggestionsExplainSuggestionItem_Click()
    ...
    If lsvSuggestions.ListItems.Count <> 0 Then
        MsgBox lsvSuggestions.SelectedItem.SubItems(1) & vbCrLf & vbCrLf & _
...
Private Sub ComputeSuggestions()
    ...
    For i = 1 To intSuggestionsCount
        ...
        Patterns = Split(suggestion_trigger, " OR ")

        'Check for the existence of one of the patterns
        For j = 0 To UBound(Patterns) - LBound(Patterns)
            If InStr(1, LastResponse, Patterns(j)) > 0 Then
