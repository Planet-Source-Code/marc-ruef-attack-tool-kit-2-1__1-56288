...
Public plugin_request As String             'Which requests shall the plugin do
Public plugin_trigger As String             'Which trigger indicates the flaw
...
Public Sub ClearAllPluginVariables()
    If frmMain.lblVulnerabilityState.BackColor <> &HE0E0E0 Then
        ...
        strAlertingText = "The vulnerability was not tested. " & _
            "Please run the selected plugin to verify the existence of the flaw."
    ...
    plugin_request = vbNullString
    plugin_trigger = vbNullString
...
Public Function ReadPluginFromFile(ByVal Filename As String) As String
...
Public Sub ParseATKPlugin(ByVal ATKPluginContent As String)
    ...
    Else
        Call PluginReadError(plugin_filename, "plugin_name")
    End If
    ...
    Else
        Call PluginReadError(plugin_filename, "plugin_protocol")
    End If
    ...
    Else
        Call PluginReadError(plugin_filename, "plugin_port")
    End If
    
    TempArray = Split(ATKPluginContent, "<plugin_request>")
    TempArray = Split(TempArray(1), "</plugin_request>")
    If LenB(TempArray(0)) <> LenB(ATKPluginContent) Then
        plugin_request = TempArray(0)
    Else
        Call PluginReadError(plugin_filename, "plugin_request")
    End If
    
    TempArray = Split(ATKPluginContent, "<plugin_trigger>")
    TempArray = Split(TempArray(1), "</plugin_trigger>")
    If LenB(TempArray(0)) <> LenB(ATKPluginContent) Then
...
Public Sub WritePluginToFile(ByRef Filename As String)
    ...
    PluginContent = _
        ...
        "<plugin_request>" & plugin_request & "</plugin_request>" & vbCrLf & _
        "<plugin_trigger>" & plugin_trigger & "</plugin_trigger>" & vbCrLf & _
    ...
    PluginContentItemCount = UBound(PluginContentArray) - LBound(PluginContentArray)
...
Public Function HowManyLoadedPlugins() As Integer
    If frmMain.filATKPlugins.ListCount <> 0 Then
        If frmMain.tvwPlugins.Nodes.Count <> 0 Then
...
Public Function GetISBNFromString(TextString As String) As String
    ...
    For i = 0 To UBound(WordArray) - LBound(WordArray)
